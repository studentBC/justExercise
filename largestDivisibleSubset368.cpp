class Solution {
public:
    void go (int start,  vector<vector<int>>& pos, vector<int>& temp, vector<int>& ans) {
        if (pos[start].size() == 0) {
            if (temp.size() > ans.size())ans = temp;
        } else {
            for (int i = 0; i < pos[start].size(); i++) {
                cout <<pos[start][i] << " , ";
                temp.emplace_back(pos[start][i]);
                go (pos[start][i], pos, temp, ans);
                temp.pop_back();
                cout << endl;
            }
        }
...
class Solution {
    int bp=0;
    vector<int> ans, furthest, best;
    
    bool passes(int val, int i, int pos) {
        if ( pos == 0 ) return true;
        if ( pos > furthest[i] && val % ans[pos-1] == 0 ) {
            furthest[i] = pos;
            return true;
        }
        return false;
    }
    void rec(int i, int pos, vector<int>& nums) {
        for ( ; i<nums.size(); ++i ) {
            if ( bp > pos && ((long)1<<(bp-pos))*nums[i] > nums.back() ) break;

            if ( passes(nums[i], i, pos) ) {
                ans[pos] = nums[i];
                rec(i+1, pos+1, nums);
            }
        }
        ...
        if ( bp < pos ) {
            bp = pos;     // store best
            best = vector<int>(ans.begin(), ans.begin()+pos);
        }
    }
public:
    vector<int> largestDivisibleSubset(vector<int>& nums) {
