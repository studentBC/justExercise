...
public:
    ...
    void postTweet(int userId, int tweetId) {
        post.emplace_back(make_pair(userId, tweetId));
    }
    ...
    vector<int> getNewsFeed(int userId) {
        vector<int>ans;
        int count = 10;
        for (int i = post.size()-1; i > -1; i--) {
            if (post[i].first == userId) {
                ans.emplace_back(post[i].second);
                count--;
            } else if (follows[userId].find(post[i].first)!= follows[userId].end()) {
                ans.emplace_back(post[i].second);
                count--;
            }
