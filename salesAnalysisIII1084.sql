...
where product_id not in (
    select if(max(sale_date) > "2019-03-31" or min(sale_date) < "2019-01-01" ,product_id, -1)
...
Product p RIGHT JOIN Sales s ON p.product_id = s.product_id
WHERE sale_date BETWEEN '2019-01-01' AND '2019-04-01'

GROUP BY p.product_id HAVING p.product_id NOT IN (
